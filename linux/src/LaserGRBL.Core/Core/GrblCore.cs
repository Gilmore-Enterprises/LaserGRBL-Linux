...
// Ported from WinForms: System.Drawing/Forms/Web removed; IGrblCoreUI + SynchronizationContext injected.
using System;
...
namespace LaserGRBL
{
	...
	public class GrblCore
	{
        ...
        public GrblCore(SynchronizationContext syncCtx, IGrblCoreUI ui, PreviewFormProxy cbform, JogFormProxy jogform)
		{
			if (Type != Firmware.Grbl) Logger.LogMessage("Program", "Load {0} core", Type);

			SetStatus(MacStatus.Disconnected);
			_syncCtx = syncCtx ?? SynchronizationContext.Current ?? new SynchronizationContext();
			mUI = ui;
		...
		protected void SetStatus(MacStatus newStatus)
		{
			lock (this)
			{
				...
				if (mMachineStatus != newStatus)
				{
					...
					if (oldStatus == MacStatus.Connecting && newStatus != MacStatus.Disconnected)
						// Sound: Connect — wired in App layer
					if (newStatus == MacStatus.Disconnected)
						// Sound: Disconnect — wired in App layer
		...
		protected void SetIssue(DetectedIssue issue)
		{
			...
			if (issue > 0) //negative numbers indicate issue caused by the user, so must not be report to UI
			{
				...
				// Sound: Fatal — wired in App layer
			}
		}
		...
		public void OpenFile(string filename = null, bool append = false)
		{
			...
			try
			{
				...
				{
					mUI?.ShowError(Strings.UnsupportedFiletype, "Error");
				}
			}
			catch (Exception ex)
			{
		...
		public void SaveProject(object parent)
		{
			if (HasProgram)
			{
				string filename = null;
		...
		public void RunProgram(object parent)
		{
			try
			{
				if (CanSendFile)
		...
		public void RunProgramFromPosition(object parent)
		{
			if (CanSendFile)
			{
				bool homing = false;
		...
		private void UserWantToContinue(object parent)
		{
		...
		public void OpenCom()
		{
			...
			catch (Exception ex)
			{
				...
				SetStatus(MacStatus.Disconnected);
				com.Close(true);
				mUI?.ShowError(ex.Message, Strings.BoxConnectErrorTitle);
			}
		}
		...
		private void OnProgramEnd()
		{
			...
			else
			{
				...
				// Sound: Success — wired in App layer
		...
		{
			get { return System.IO.Path.GetDirectoryName(System.Reflection.Assembly.GetEntryAssembly()?.Location ?? AppContext.BaseDirectory); }
		}
		...
		internal void HKDisconnect()
		{
			if (IsConnected)
			{
				if (!(InProgram && !(mUI?.AskConfirmation(Strings.DisconnectAnyway, Strings.WarnMessageBoxHeader) ?? false)))
					CloseCom(true);
			}
		}
	...
	public class TimeProjection
	{
		...
		public void JobError()
		{
			if (mStarted && !mCompleted)
			{
				// Sound: Warning — wired in App layer
				mErrorCount++;
			}
		}
		...
		//private string LastJobFileName => System.IO.Path.Combine(AppPaths.DataPath, "lastjob.nc");
		//private string LastPositionFileName => System.IO.Path.Combine(AppPaths.DataPath, "jobprogress.dat");
	...
	public static class LaserLifeHandler
	{
		...
		static LaserLifeHandler()
		{
			if (mLLCL == null)
			{
				mLLCFileName = System.IO.Path.Combine(AppPaths.DataPath, "LaserLifeCounter.bin");
		...
		public static void OnConnect(object parent)
		{
			try
			{
		...
		static string EscapeJson(object o)
		{
			if (o is double?)
				...
				return System.Text.RegularExpressions.Regex.Replace(o != null ? o.ToString() : ""?.ToString() ?? "", @"[""\\]", m => @"\" + m.Value);
		}
