...
// System.Windows (WPF/WindowsBase) removed: System.Windows.Point → CorePoint.
        ...
        public class G2G3Helper
		{
			...
			public G2G3Helper(LaserGRBL.GrblCommand.StatePositionBuilder spb, LaserGRBL.GrblCommand cmd)
			{
				...
				if (Circle(aX, aY, bX, bY))
					BBox = new CoreRect(RectX, RectY, RectW, RectH);
				else
			...
			public static CoreRect BBBox(Double startAngle, Double endAngle, Double r, Double centerX, Double centerY)
			{
				int startQuad = GetQuadrant(startAngle) - 1;
				int endQuad = GetQuadrant(endAngle) - 1;

				// Convert to Cartesian coordinates.
				CorePoint stPt = new CorePoint(Math.Round(r * Math.Cos(startAngle), 14), Math.Round(r * Math.Sin(startAngle), 14));
				CorePoint enPt = new CorePoint(Math.Round(r * Math.Cos(endAngle), 14), Math.Round(r * Math.Sin(endAngle), 14));
				...
				CorePoint startPt = new CorePoint(xMin[endQuad, startQuad], yMin[endQuad, startQuad]);
				CorePoint endPt = new CorePoint(xMax[endQuad, startQuad], yMax[endQuad, startQuad]);
				
				CoreRect rv = new CoreRect(startPt, endPt);
				// Arc computed relative to center at 0,0 — shift to real center.
				rv.X += centerX;
				rv.Y += centerY;
