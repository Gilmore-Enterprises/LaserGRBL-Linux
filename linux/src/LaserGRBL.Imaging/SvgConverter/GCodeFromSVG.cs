...
// CorePoint from LaserGRBL namespace (replacing System.Windows.Point)
...
namespace LaserGRBL.SvgConverter
{
	...
	class GCodeFromSVG
	{
		...
		private bool svgClosePathExtend = true;  // not working if true move to first and second CorePoint of path to get overlap
		...
		public CorePoint UserOffset = new CorePoint(0,0);
		...
		private GdiMatrix[] matrixGroup = new GdiMatrix[10];   // store SVG-Group transformation matrixes
		private GdiMatrix matrixElement = new GdiMatrix();     // store finally applied GdiMatrix
		private GdiMatrix oldMatrixElement = new GdiMatrix();     // store finally applied GdiMatrix
		...
		{
			// Embedded resources moved to App layer; SVG files are always external here.
		...
		{  
			...
			GdiMatrix tmp = new GdiMatrix(1, 0, 0, 1, 0, 0); // m11, m12, m21, m22, offsetx, offsety
			svgWidthPx = 0;
			svgHeightPx = 0;
			float vbOffX = 0;
			float vbOffY = 0;
			...
			tmp.OffsetX += (float)UserOffset.X; //add user offset for centerline
			tmp.OffsetY += (float)UserOffset.Y; //add user offset for centerline
		...
		/// </summary>
		private GdiMatrix parseTransform(XElement element, bool isGroup, int level)
		{
			GdiMatrix finalMatrix = new GdiMatrix(1, 0, 0, 1, 0, 0); // m11, m12, m21, m22, offsetx, offsety
			if (element.Attribute("transform") != null)
			{
				...
				{
					if (word.StartsWith("late")) { finalMatrix = GdiMatrix.Multiply(ParseTransform("trans" + word), finalMatrix); }
					else if (word.StartsWith("le")) { finalMatrix = GdiMatrix.Multiply(ParseTransform("sca" + word), finalMatrix); }
					else if (word.StartsWith("ate")) { finalMatrix = GdiMatrix.Multiply(ParseTransform("rot" + word), finalMatrix); }
					else if (word.StartsWith("rix")) { finalMatrix = GdiMatrix.Multiply(ParseTransform("mat" + word), finalMatrix); }
				}
			}
			if (isGroup)
			{
				...
				if (level > 0)
				{
					...
					{ matrixGroup[i] = GdiMatrix.Multiply(finalMatrix, matrixGroup[level - 1]); }
				}
				else
				{   //tmp.M22 = -tmp.M22;
			...
			{ matrixElement = GdiMatrix.Multiply(finalMatrix, matrixGroup[level]); }

			return finalMatrix;
		}

		private static GdiMatrix ParseTransform(string transform)
		{
			GdiMatrix tmp = new GdiMatrix(1, 0, 0, 1, 0, 0); // m11, m12, m21, m22, offsetx, offsety
			...
			if ((transform != null) && (transform.IndexOf("rotate") >= 0))
			{
				...
				float px = split.Length == 3 ? ConvertToPixel(split[1]) : 0.0f; //<--- this read rotation offset CorePoint x
				float py = split.Length == 3 ? ConvertToPixel(split[2]) : 0.0f; //<--- this read rotation offset CorePoint y
				tmp.RotateAt(angle, px, py); // <--- this apply RotateAt GdiMatrix
			}
			if ((transform != null) && (transform.IndexOf("GdiMatrix") >= 0))
			{
				var coord = GetTextBetween(transform, "GdiMatrix(");//, ")");
				var split = coord.Split(',');
				if (coord.IndexOf(',') < 0)
					split = coord.Split(' ');
		...
		{       // https://www.w3.org/TR/SVG/coords.html#Units          // in=90 or 96 ???
			...
			else if (str.IndexOf("pt") > 0) { factor = factor_Pt2Px; }          // CorePoint
				...
				foreach (var pathElement in svgCode.Elements(nspace + form))
				{
					if (filterByColor(pathElement, loadColorFilter))
					{
						...
						if (svgComments)
						{
							...
							gcodeString.AppendFormat("( SVG CoreColor=#{0})\r\n", myColor);
						}
						...
						if (form == "rect")
						{
							...
							if (rx > 0)
							{
								...
								gcodeMoveTo(x + rx, y + height, form);  // repeat first CorePoint to avoid back draw after last G3
							}
							gcodeStopPath(form);
						}
						else if (form == "circle")
		...
		private bool filterByColor(XElement pathElement, ColorFilter filter)
		{
			...
			if (rgb == CoreColor.Transparent) return true;
		...
		private CoreColor parseRgb(string style)
		{
			// ColorTranslator.FromHtml (GDI+) → parse hex color manually.
			if (string.IsNullOrWhiteSpace(style)) return CoreColor.Transparent;
			style = style.Trim();
			if (style.StartsWith("#") && (style.Length == 7 || style.Length == 4))
			{
				try
				{
					if (style.Length == 4) // #RGB → #RRGGBB
						style = $"#{style[1]}{style[1]}{style[2]}{style[2]}{style[3]}{style[3]}";
					byte r = Convert.ToByte(style.Substring(1, 2), 16);
					byte g = Convert.ToByte(style.Substring(3, 2), 16);
					byte b = Convert.ToByte(style.Substring(5, 2), 16);
					return new CoreColor(r, g, b);
				}
				catch { }
			}
			// Named colors — use SkiaSharp to parse
			var sk = SKColor.Parse(style);
			return new CoreColor(sk.Red, sk.Green, sk.Blue, sk.Alpha);
		}
		...
		private int parsePathCommand(string svgPath)
		{
			...
			switch (cmd)
			{
				...
				case 'L':       // Draw a line from the current CorePoint to the given (x,y) coordinate
					for (int i = 0; i < floatArgs.Length; i += 2)
					{
						objCount++;
						if (absolute)
				...
				case 'H':       // Draws a horizontal line from the current CorePoint (cpx, cpy) to (x, cpy)
				...
				case 'V':       // Draws a vertical line from the current CorePoint (cpx, cpy) to (cpx, y)
					for (int i = 0; i < floatArgs.Length; i++)
					{
						objCount++;
						if (absolute)
				...
				case 'A':       // Draws an elliptical arc from the current CorePoint to (x, y)
				...
				case 'C':       // Draws a cubic Bézier curve from the current CorePoint to (x,y)
					if (svgComments) { gcodeString.AppendFormat("( Command {0} {1} )\r\n", command.ToString(), ((absolute == true) ? "absolute" : "relative")); }
					for (int rep = 0; rep < floatArgs.Length; rep += 6)
					{
						...
						if ((rep + 5) < floatArgs.Length)
						{
							...
							var points = new CorePoint[4];
							points[0] = new CorePoint(lastX, lastY);
							points[1] = new CorePoint(cx1, cy1);
							points[2] = new CorePoint(cx2, cy2);
							points[3] = new CorePoint(cx3, cy3);
				...
				case 'S':       // Draws a cubic Bézier curve from the current CorePoint to (x,y)
					if (svgComments) { gcodeString.AppendFormat("( Command {0} {1} )\r\n", command.ToString(), ((absolute == true) ? "absolute" : "relative")); }
					for (int rep = 0; rep < floatArgs.Length; rep += 4)
					{
						...
						var points = new CorePoint[4];
						points[0] = new CorePoint(lastX, lastY);
						points[1] = new CorePoint(cxMirror, cyMirror);
						points[2] = new CorePoint(cx2, cy2);
						points[3] = new CorePoint(cx3, cy3);
						var b = GetBezierApproximation(points, svgBezierAccuracy);
						if (svgNodesOnly)
				...
				case 'Q':       // Draws a quadratic Bézier curve from the current CorePoint to (x,y)
					if (svgComments) { gcodeString.AppendFormat("( Command {0} {1} )\r\n", command.ToString(), ((absolute == true) ? "absolute" : "relative")); }
					for (int rep = 0; rep < floatArgs.Length; rep += 4)
					{
						objCount++;
						...
						var points = new CorePoint[4];
						points[0] = new CorePoint(lastX, lastY);
						points[1] = new CorePoint(qpx1, qpy1);
						points[2] = new CorePoint(qpx2, qpy2);
						points[3] = new CorePoint(cx3, cy3);
						cxMirror = cx3 - (cx2 - cx3); cyMirror = cy3 - (cy2 - cy3);
						lastX = cx3; lastY = cy3;
						var b = GetBezierApproximation(points, svgBezierAccuracy);
				...
				case 'T':       // Draws a quadratic Bézier curve from the current CorePoint to (x,y)
					if (svgComments) { gcodeString.AppendFormat("( Command {0} {1} )\r\n", command.ToString(), ((absolute == true) ? "absolute" : "relative")); }
		...
		private void calcArc(float StartX, float StartY, float RadiusX, float RadiusY, float Angle, float Size, float Sweep, float EndX, float EndY)
		{
			...
			for (int i = 0; i < segments; ++i)
			{
				...
				var points = new CorePoint[4];
				points[0] = new CorePoint(startX, startY);
				points[1] = new CorePoint((startX + dx1), (startY + dy1));
				points[2] = new CorePoint((endpointX + dxe), (endpointY + dye));
				points[3] = new CorePoint(endpointX, endpointY);
				var b = GetBezierApproximation(points, svgBezierAccuracy);
		...
		/// </summary>
		private CorePoint[] GetBezierApproximation(CorePoint[] controlPoints, int outputSegmentCount)
		{
			if (UseLegacyBezier)
        ...
        private CorePoint[] GetBezierApproximationOld(CorePoint[] controlPoints, int outputSegmentCount)
		{
			CorePoint[] points = new CorePoint[outputSegmentCount + 1];
		...
		private CorePoint GetBezierPoint(double t, CorePoint[] controlPoints, int index, int count)
		{
			if (count == 1)
			...
			return new CorePoint(x, (1 - t) * P0.Y + t * P1.Y);
		}
		...
		/// <summary>
		/// Transform XY coordinate using GdiMatrix and scale  
		/// </summary>
		/// <param name="pointStart">coordinate to transform</param>
		/// <returns>transformed coordinate</returns>
		private CorePoint translateXY(float x, float y)
		{
			CorePoint coord = new CorePoint(x, y);
			return translateXY(coord);
		}
		private CorePoint translateXY(CorePoint pointStart)
		{
			CorePoint pointResult = matrixElement.MapPoint(pointStart);
			return pointResult;
		}
		/// <summary>
		/// Transform I,J coordinate using GdiMatrix and scale  
		/// </summary>
		/// <param name="pointStart">coordinate to transform</param>
		/// <returns>transformed coordinate</returns>
		private CorePoint translateIJ(float i, float j)
		{
			CorePoint coord = new CorePoint(i, j);
			return translateIJ(coord);
		}
		private CorePoint translateIJ(CorePoint pointStart)
		{
			double tmp_i = pointStart.X, tmp_j = pointStart.Y;
			return new CorePoint(
				tmp_i * matrixElement.M11 + tmp_j * matrixElement.M21,
				tmp_i * matrixElement.M12 + tmp_j * matrixElement.M22);
			// i,j are relative - no offset needed, but rotation is applied
		}
		...
		{
			CorePoint coord = translateXY(x, y);
		...
		private void gcodeStopPath(string cmt)
		{
			if (gcodeReduce)
			{
				if ((lastSetGCX != lastGCX) || (lastSetGCY != lastGCY)) // restore last skipped CorePoint for accurat G2/G3 use
					gcode.MoveTo(gcodeString, new CorePoint(lastGCX, lastGCY), "restore CorePoint");
			}
			gcodePenUp(cmt);
		}
		...
		/// Insert G1 gcode command
		/// </summary>
		private void gcodeMoveTo(CorePoint orig, string cmt)
		{
			CorePoint coord = translateXY(orig);
			rejectPoint = false;
			gcodePenDown(cmt);
		...
		{
			CorePoint coordxy = translateXY(x, y);
			CorePoint coordij = translateIJ(i, j);
			gcodePenDown(cmt);
			if (gcodeReduce && isReduceOk)      // restore last skipped CorePoint for accurat G2/G3 use
			{
				if ((lastSetGCX != lastGCX) || (lastSetGCY != lastGCY))
					gcode.MoveTo(gcodeString, new CorePoint(lastGCX, lastGCY), cmt);
			}
