...
namespace LaserGRBL.SvgConverter
{
    ...
    internal static class BezierTools
    {
        ...
        /// 
        /// This code is a modified version of what was found in the nearest CorePoint algorithm 
        ...
        private static Tuple<CorePoint [], CorePoint[]> SplitCurveAtT(CorePoint[] controlPoints, double t)
        {
            var degree = controlPoints.Length - 1;
            CorePoint[,] Vtemp = new CorePoint[degree + 1, degree + 1];
            ...
            for (var i = 1; i <= degree; i++)
            {
                ...
                {
                    Vtemp[i, j] = new CorePoint(
                        (1.0 - t) * Vtemp[i - 1, j].X + t * Vtemp[i - 1, j + 1].X,
                        (1.0 - t) * Vtemp[i - 1, j].Y + t * Vtemp[i - 1, j + 1].Y);
                }
            }

            var result = new Tuple<CorePoint[], CorePoint[]>(
                new CorePoint[degree + 1],
                new CorePoint[degree + 1]
            );
        ...
        /// <returns></returns>
        private static IEnumerable<CorePoint> FlattenSegmentTo(IEnumerable<CorePoint> points, double error, int subdivisions, int maxSubdivisions)
        {
        ...
        private static double CalculateTriangleArea(CorePoint a, CorePoint b, CorePoint c)
        ...
        /// the last CorePoint on one curve is the first CorePoint of the next.
        /// 
        ...
        public static IEnumerable<CorePoint> FlattenTo(IList<CorePoint> points, double error=0.01, int maxSubdivisions=20)
        {
            var result = new List<CorePoint> { points.First() };
