...
namespace LaserGRBL.SvgConverter
{
	public static class gcode
	{
		...
		public static void MoveTo(StringBuilder gcodeString, CorePoint coord, string cmt = "")
		...
		public static void MoveToRapid(StringBuilder gcodeString, CorePoint coord, string cmt = "")
		{
		...
		private static void MoveSplit(StringBuilder gcodeString, int gnr, float finalx, float finaly, float? z, bool applyFeed, string cmt)
		{
			...
			{
				CorePoint tmp = dragToolCompensation(gcodeString, finalx, finaly);
			...
			//	//auch nach G0 move
			//	if (Properties.Settings.Default.importGCSubFirst && (lastMovewasG0 || (moveLength >= segmentLength)))       // also subroutine at first CorePoint
			//	{
		...
		private static CorePoint dragToolCompensation(StringBuilder gcodeString, float finalx, float finaly)
		{
			float dx = finalx - origFinalX;// lastx;       // remaining distance until full move
			float dy = finaly - origFinalY;// lasty;       // lastXY is global
			float moveLength = (float)Math.Sqrt(dx * dx + dy * dy);
			if (moveLength == 0)
				return new CorePoint(finalx, finaly);
		...
		public static void Arc(StringBuilder gcodeString, int gnr, CorePoint coordxy, CorePoint coordij, string cmt = "", bool avoidG23 = false)
		...
		public static void splitArc(StringBuilder gcodeString, int gnr, float x1, float y1, float x2, float y2, float i, float j, bool applyFeed, string cmt = "")
		{
			...
			float cx = x1 + i, cy = y1 + j;                                 // get center CorePoint of circle
