...
namespace CsPotrace
{
	class PotraceClipper
	{
		...
		private static void AddGridClip(List<List<Curve>> plist, Clipper c)
		{
			...
			{
				PotracePathAdapter Current = new PotracePathAdapter();
				for (int j = 0; j < LC.Count; j++)
				{
					Curve C = LC[j];
					if (C.Kind == CurveKind.Line)
					{
						Current.AddLine(new CorePoint((float)C.A.X, (float)C.A.Y), new CorePoint((float)C.B.X, (float)C.B.Y));
					}
					else
					{
						CorePoint A = new CorePoint((float)C.A.X, (float)C.A.Y);
						Current.CubicTo(new CorePoint((float)C.A.X, (float)C.A.Y), new CorePoint((float)C.ControlPointA.X, (float)C.ControlPointA.Y), new CorePoint((float)C.ControlPointB.X, (float)C.ControlPointB.Y), new CorePoint((float)C.B.X, (float)C.B.Y));
					}
				}
		...
		static void AddClip(Clipper c, PotracePathAdapter psubject)
		{
			psubject.Flatten();

			List<IntPoint> subject = new List<IntPoint>(psubject.PathPoints.Count);
			foreach (CorePoint p in psubject.PathPoints)
				subject.Add(new IntPoint(p.X * resolution, p.Y * resolution));

			c.AddPath(subject, PolyType.ptClip, true);
		}

		static void AddClip(ClipperOffset c, PotracePathAdapter psubject)
		{
			psubject.Flatten();
		...
		static void AddGridSubject(Clipper c, double w, double h, LaserGRBL.GrblFile.L2LConf cnf)
		{
			...
			if (dir == LaserGRBL.RasterConverter.ImageProcessor.Direction.NewHilbert)
			{
				...
				List <CorePoint> texel = Hilbert.Execute(n, (float)(step));
		...
		internal class Hilbert
		{
			...
			public static List<CorePoint> Execute(int depth, float delta)
			{
				List<CorePoint> rv = new List<CorePoint>();
				float dy = 0;
				float dx = delta;
				rv.Add(new CorePoint(0, 0));
			...
			private void DoHilbert(List<CorePoint> list, int depth, float dx, float dy)
			{
				if (depth > 1) DoHilbert(list, depth - 1, dy, dx);
				list.Add(new CorePoint(list[list.Count - 1].X + dx, list[list.Count - 1].Y + dy));
				if (depth > 1) DoHilbert(list, depth - 1, dx, dy);
				list.Add(new CorePoint(list[list.Count - 1].X + dy, list[list.Count - 1].Y + dx));
				if (depth > 1) DoHilbert(list, depth - 1, dx, dy);
				list.Add(new CorePoint(list[list.Count - 1].X - dx, list[list.Count - 1].Y - dy));
				if (depth > 1) DoHilbert(list, depth - 1, -dy, -dx);
			}
