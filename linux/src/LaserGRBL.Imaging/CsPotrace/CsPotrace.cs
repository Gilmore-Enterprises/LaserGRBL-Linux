...
{  /// <summary>
	...
	public partial class Potrace
	{
		...
		static void ConvertBitmap(ICoreBitmap bitmap)
		{
			// SkiaSharp port: use ICoreBitmap.GetPixelGray (0=white, 255=black).
			int w = bitmap.Width, h = bitmap.Height;
			byte[] Result = new byte[w * h];
			for (int y = 0; y < h; y++)
			for (int x = 0; x < w; x++)
			{
				// GetPixelGray returns inverted luminance (255 = black = laser on).
				// Potrace expects 1 = foreground (dark), 0 = background (white).
				Result[y * w + x] = (byte)(bitmap.GetPixelGray(x, y) > (int)(Treshold * 255) ? 1 : 0);
			}
			bm = new Bitmap_p(w, h);
			bm.data = Result;
		}
	 ...
	 segment */
	   ...
	   on failure. */
		...
		public static List<List<Curve>> PotraceTrace(ICoreBitmap Bitmap)
		{
			Clear();
